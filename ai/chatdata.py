...
if DATA_MODE == "mysql":
    ...
    def search_recipes(ingredient: str, limit: int = 5):
        """Tìm món trong bảng recipes có chứa nguyên liệu (LIKE, tham số hóa - A03)."""
        like = f"%{ingredient.lower().strip()[:100]}%"
        rows = _q("SELECT recipe_id, name FROM recipes "
                  "WHERE LOWER(name) LIKE %s OR LOWER(CAST(ingredients AS CHAR)) LIKE %s "
                  "LIMIT %s", (like, like, int(limit)), "all") or []
        return [{"recipe_id": r["recipe_id"], "name": r["name"]} for r in rows]
    ...
    _RECIPES_STUB = [
        {"recipe_id": 1, "name": "Grilled chicken breast salad", "ingredients": "chicken breast, lettuce, tomato, oil"},
        {"recipe_id": 2, "name": "Chicken fried rice",           "ingredients": "rice, chicken breast, egg, onion"},
        {"recipe_id": 3, "name": "Beef and potato stew",         "ingredients": "beef, potato, carrot, onion"},
        {"recipe_id": 4, "name": "Tomato egg stir-fry",          "ingredients": "egg, tomato, oil, salt"},
    ]
    def search_recipes(ingredient: str, limit: int = 5):
        key = ingredient.lower().strip()
        hits = [r for r in _RECIPES_STUB if key in r["ingredients"] or key in r["name"].lower()]
        return [{"recipe_id": r["recipe_id"], "name": r["name"]} for r in hits[:limit]]